...
- 手表四页：下一节课／进行中、自动日课表、最近 3 项未完成作业、未来 5 天考试。手表只同步和显示，没有编辑、完成、宠物或教学网操作。列表 1–3 项时完整显示且没有页内滚动空间，4 项及以上保留页内滚动；较长内容可点开查看详情。
...
多台手机可连接时选择目标设备，或通过 `--device "你的 iPhone 名称"` 指定。脚本先检查设备连接与锁屏状态，再构建并签名，再停止手机应用的后台进程、备份并校验数据、覆盖安装；首次启动前会核对数据库是否保留。备份期间不要打开手机上的 Schedule，安装完成后再断开连接。
...
| 已配对但连接超时 | “已配对，待连接”不代表调试通道已连通。解锁手机和手表，开启 Mac、手机和手表的 Wi-Fi，确认本地网络互通；在 Xcode 的 Devices and Simulators 查看准备状态。连接检查失败时尚未开始备份或安装。 |
| `build-watch` 退出码 70，等待 destination 超时 | 脚本仅针对这类准备超时改用通用设备目标构建，仍会检查签名是否包含所选设备；实际安装仍需要设备连通。编译或签名错误不会跳过。 |
